...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '28px 32px', flex: 1 }}>
          ...
          {[['Foto totali', photos.length, '📷'], ['Ospiti', guestList.length, '👥'], ['Selezionate', selected.size, '✓']].map(([label, num, icon]) => (
            <div key={label} style={{ background: 'white', borderRadius: 16, padding: '20px 24px', border: '1px solid var(--border)', boxShadow: '0 2px 8px rgba(196,116,138,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</span>
              </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <select style={{ padding: '8px 14px', background: 'white', border: '1.5px solid var(--border)', borderRadius: 100, color: 'var(--text)', fontFamily: 'var(--font-body)', fontSize: 13, cursor: 'pointer', outline: 'none' }}
            ...
            <button className="btn btn-ghost" onClick={fetchPhotos} disabled={loading} style={{ fontSize: 12, borderRadius: 100 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            {filtered.map(photo => (
              <div key={photo.id} onClick={() => setLightbox(photo)}
                ...
                {photo.thumbnailUrl
                  ? <img src={photo.thumbnailUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" />
                  : <div style={{ width: '100%', height: '100%', background: 'var(--rose-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>📷</div>}
                <button onClick={e => { e.stopPropagation(); toggleSelect(photo.id) }}
                  style={{ position: 'absolute', top: 8, right: 8, width: 24, height: 24, borderRadius: '50%', border: `2px solid ${selected.has(photo.id) ? 'var(--rose)' : 'rgba(255,255,255,0.8)'}`, background: selected.has(photo.id) ? 'var(--rose)' : 'rgba(255,255,255,0.5)', cursor: 'pointer', color: selected.has(photo.id) ? 'white' : 'transparent', fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, backdropFilter: 'blur(4px)', transition: 'all 0.2s' }}>
      ...
      {/* Lightbox */}
      {lightbox && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(44,36,36,0.85)', backdropFilter: 'blur(12px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease' }}
          onClick={() => setLightbox(null)}>
          <div style={{ background: 'white', borderRadius: 20, overflow: 'hidden', maxWidth: 860, width: '100%', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.25)' }}
            onClick={e => e.stopPropagation()}>
            {lightbox.url
              ? <img src={lightbox.url} alt="" style={{ width: '100%', maxHeight: '65vh', objectFit: 'contain', background: '#F5F5F5' }} />
              : <div style={{ height: 300, background: 'var(--rose-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 64 }}>📷</div>}
