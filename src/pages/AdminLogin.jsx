...
export default function AdminLogin() {
  ...
  const handleLogin = async () => {
    ...
    if (username === 'Admin' && password === 'Chiara18!Admin') {
      loginAdmin(); navigate('/dashboard', { replace: true })
    } else {
      addToast('Credenziali non valide', 'error'); setLoading(false)
    }
  }
      ...
      <div style={{ width:'100%', maxWidth:420, background:'white', borderRadius:24, padding:'52px 44px', boxShadow:'0 12px 48px rgba(196,116,138,0.12)', border:'1px solid var(--border)' }} className="fade-up">
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:28 }}>
          <div>
            ...
            <input className="input" type="text" placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} disabled={loading} />
          </div>
          <div>
            ...
            <div style={{ position:'relative' }}>
              <input className="input" type={showPass ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} disabled={loading} style={{ paddingRight:36 }} />
              <button type="button" onClick={() => setShowPass(!showPass)} style={{ position:'absolute', right:0, bottom:10, background:'none', border:'none', cursor:'pointer', fontSize:16, color:'var(--text-muted)' }}>{showPass ? '🙈' : '👁️'}</button>
            </div>
          </div>
          <button className="btn btn-rose" style={{ width:'100%', justifyContent:'center', padding:14 }} onClick={handleLogin} disabled={loading}>
